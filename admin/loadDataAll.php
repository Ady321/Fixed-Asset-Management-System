<?php
    ...
    if(isset($_POST['byAsset'])){
            
        $stmt1 = mysqli_query($db, "select a.loc_id,l.Location_name,u.user_Name,a.TagId,c.cat_Name,c.Color,a.asset_name,m.manu_name,sc.Name,a.serial_no,ac.Conditions from asset a,sub_category sc,asset_condition ac,category c,manufactures m,users u,locations l where l.loc_id=a.loc_id and u.Id=a.Id and a.con_id=ac.con_id and sc.sub_id=a.sub_id and sc.category_Id=c.category_Id and m.manu_id=sc.manu_id");
        if(mysqli_num_rows($stmt1)>0){
            $output.= '<form method="POST" action="exportData2.php">';
            ...
            $output.='</tbody></table>
            <input type="submit" name="export2" value="Export xls By Asset" class="btn btn-success"></form>';
