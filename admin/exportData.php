	...
	if(!$user->is_logged_in()){ header('Location: login'); }
        ...
        if(isset($_POST['export'])){
            ...
            $stmt= mysqli_query($db,"select u.job_title,d.Name as dept_name,u.user_Name,l.Location_name,a.quan,sc.Price,sc.warranty_end,sc.Purchase_date,sc.Supplier_date,sc.warranty_start,s.sup_Name,a.TagId,a.serial_no,c.Color,c.cat_Name,a.asset_name,m.manu_name,sc.Name,ac.Conditions from users u,suppliers s,locations l,asset a,sub_category sc,asset_condition ac,category c,manufactures m,dept_teams d where d.dept_id=u.dept_id and a.sup_id=s.sup_id and a.loc_id=l.loc_id and a.con_id=ac.con_id and sc.sub_id=a.sub_id and 
                    sc.category_Id=c.category_Id and m.manu_id=sc.manu_id and a.loc_id='".$lol."' and u.Id=a.Id");
            ...
            while ($row_loc = mysqli_fetch_array($stmt)){
                $pu_date= date('jS M Y', strtotime($row_loc['Purchase_date']));
                $su_date= date('jS M Y', strtotime($row_loc['Supplier_date']));
                $wst_date= date('jS M Y', strtotime($row_loc['warranty_start']));
                $wen_date= date('jS M Y', strtotime($row_loc['warranty_end']));
                ...
                $output.= '<td>'.$row_loc['asset_name'].'</td>
                           <td>'.$row_loc['Name'].'</td>
                           <td>'.$row_loc['manu_name'].'</td>
                    <td>'.$row_loc['Conditions'].'</td>
                    <td>'.$row_loc['Location_name'].'</td>
                    <td>'.$row_loc['Price'].'</td>
                    <td>'.$row_loc['quan'].'</td>
                    <td>'.$pu_date.'</td>
                    <td>'.$row_loc['sup_Name'].'</td>
                    <td>'.$su_date.'</td>
                    <td>'.$wst_date.'</td>
                    <td>'.$wen_date.'</td>
                    <td>'.$row_loc['user_Name'].'</td>
                    <td>'.$row_loc['job_title'].'</td>
                    <td>'.$row_loc['dept_name'].'</td></tr>';
            }
            $output.='</tbody></table>';
             header('Content-Type: application/xls');
             header('Content-Disposition: attachment; filename=Report_By_Location.xls');
             echo $output;
